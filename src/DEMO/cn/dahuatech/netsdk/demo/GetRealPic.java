package DEMO.cn.dahuatech.netsdk.demo;

import com.sun.jna.NativeLong;
import com.sun.jna.Pointer;

import cn.dahuatech.netsdk.*;
...
// 断线回调
class fDisConnectCB implements DHNetSDKLib.fDisConnect{
...
// 网络连接恢复回调函数原形
class fHaveReConnectCB implements DHNetSDKLib.fHaveReConnect{
 public int invoke(NativeLong lLoginID, String pchDVRIP, int nDVRPort, NativeLong dwUser){
	 // 停止订阅用于释放资源
...
// 智能实时信息及图片回调接口
class fAnalyzerDataCallBackCB implements DHNetSDKLib.fAnalyzerDataCallBack{
...
public class GetRealPic{
	...
	public static void main(String[] args) throws InterruptedException{
		...
		// 初始化
		DHNetSDKLibObj.CLIENT_Init(new fDisConnectCB(), new NativeLong(0));
		System.out.println("dhnetsdk Init ok !");
		DHNetSDKLibObj.CLIENT_SetAutoReconnect(new fHaveReConnectCB(), new NativeLong(0));
...
//		}
	...
	// 常量
	private static DHNetSDKLib.NET_DEVICEINFO m_stDeviceInfo ;
	...
	public static void Login(String sIP) {
		...
		m_stDeviceInfo = new DHNetSDKLib.NET_DEVICEINFO();
		...
		int nError[] = {0};
		m_lNativeLoginID = DHNetSDKLibObj.CLIENT_LoginEx(m_strIP, m_sPort, m_strUserName,m_strUserPwd,nSpecCap,pCapParam, m_stDeviceInfo,nError);
		
		System.out.printf("[%d],%s:%d login %s, device type is %d, channal num is %d , err: %d, CLIENT_GetLastError:%d !\n",
				m_lNativeLoginID.longValue(),m_strIP,m_sPort,m_lNativeLoginID.longValue()>0?"ok":"failure",m_stDeviceInfo.byDVRType, m_stDeviceInfo.byChanNum, nError[0], DHNetSDKLibObj.CLIENT_GetLastError());
		if(0 == m_lNativeLoginID.longValue())
		{
	...
	public static void Logout() {
		...
		boolean bRet = DHNetSDKLibObj.CLIENT_Logout(m_lNativeLoginID);
	...
	public static void StopRealPic(){
		...
		{
			DHNetSDKLibObj.CLIENT_StopLoadPic(m_lNativeRealPicHandle);
	...
	public static void getRealPic()
	{
		...
		int dwAlarmType = DHNetSDKLib.EVENT_IVS_ALL; // 默认全部报警订阅
		boolean bNeedPicFile = true; // 订阅上传图片,只要报警事件信息填false
		NativeLong dwUser = new NativeLong(0);
		Pointer Reserved = null;
		
		m_lNativeRealPicHandle = DHNetSDKLibObj.CLIENT_RealLoadPictureEx(
				m_lNativeLoginID,
				nChannelID,
				dwAlarmType,
				bNeedPicFile,
		...
		System.out.printf("[%d],real load picture %s, CLIENT_GetLastError:%d !\n",
				m_lNativeRealPicHandle.longValue(),m_lNativeRealPicHandle.longValue()>0?"ok":"failure",DHNetSDKLibObj.CLIENT_GetLastError());
	...
	public static void event_ivs_traffic_runredlight_func(Pointer pAlarmInfo, Pointer pBuf, int dwBufSize)
	{
		...
		DHNetSDKLib.DEV_EVENT_TRAFFIC_RUNREDLIGHT_INFO pRunRed = new DHNetSDKLib.DEV_EVENT_TRAFFIC_RUNREDLIGHT_INFO();
		pRunRed.write();
	...
	public static void event_ivs_traffic_manualsnap_func(Pointer pAlarmInfo, Pointer pBuf, int dwBufSize)
	{
		...
		DHNetSDKLib.DEV_EVENT_TRAFFIC_MANUALSNAP_INFO pManualSnap = new DHNetSDKLib.DEV_EVENT_TRAFFIC_MANUALSNAP_INFO();
	...
	public static void event_ivs_trafficgate_func(Pointer pAlarmInfo, Pointer pBuf, int dwBufSize)
	{
		...
		DHNetSDKLib.DEV_EVENT_TRAFFICGATE_INFO pGate = new DHNetSDKLib.DEV_EVENT_TRAFFICGATE_INFO();
		pGate.write();
	...
	public static void event_ivs_trafficjunction_func(Pointer pAlarmInfo, Pointer pBuf, int dwBufSize)
	{
		...
		DHNetSDKLib.DEV_EVENT_TRAFFICJUNCTION_INFO pJunction = new DHNetSDKLib.DEV_EVENT_TRAFFICJUNCTION_INFO();
		pJunction.write();
